...
interface ItemData {
  ...
  price?: number;
  type?: string;
  link: string; // ✅ إضافة حقل الرابط المباشر
}
...
export default function DynamicShowcase({ title, linkHref, data, dataType }: DynamicShowcaseProps) {
  ...
  // ✅ تنسيق البيانات بشكل موحد ودقيق حسب النوع
  const formattedData: ItemData[] = data.map((item) => {
    let imagesList: string[] = [];
    
    // التعامل مع الصور بناءً على نوع البيانات
    if (dataType === "places") {
      imagesList = item.media_urls || [];
    } else {
      // للخدمات، عادة تكون صورة واحدة في image_url
      imagesList = item.media_urls && item.media_urls.length > 0 
                    ? item.media_urls 
    ...
    return {
      ...
      images: imagesList,
      location: item.city || "عسير",
      price: item.price,
      type: dataType === "places" ? item.type : item.service_category,
      // ✅ تحديد الرابط القطعي بناءً على الـ dataType
      link: dataType === "places" ? `/place/${item.id}` : `/service/${item.id}`
    };
  });
  ...
  useEffect(() => {
    ...
    // التقليب كل 10 ثواني (إذا كان هناك أكثر من 3 عناصر)
    if (formattedData.length > 3) {
      const interval = setInterval(() => {
        setVisibleItems((prev) => {
          if (formattedData.length > 0) {
              const newArray = [...formattedData];
              newArray.push(newArray.shift()!); // نقل أول عنصر للآخر
...
function ShowcaseCard({ item }: { item: ItemData }) {
  ...
  return (
    // ✅ استخدام الرابط القطعي اللي جهزناه في الأعلى
    <Link href={item.link} className="block h-full">
      <div className="group h-96 relative bg-[#1a1a1a] rounded-2rem overflow-hidden border border-white/10 transition-all duration-700 hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#C89B3C]/10">
        <div className="relative h-full w-full">
          {item.images.length > 0 ? item.images.map((img, idx) => (
              <Image 
                key={idx} 
                src={img} 
                alt={item.title || "صورة العرض"} 
                fill 
                className={`object-cover transition-opacity duration-1000 ${idx === currentImageIndex ? "opacity-100 scale-105" : "opacity-0 scale-100"}`} 
              />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-20 text-right">
            <h3 className="text-xl font-bold text-white group-hover:text-[#C89B3C] transition line-clamp-2">{item.title}</h3>
