...
export default function PlaceDetails() {
  ...
  const toggleFavorite = async () => {
    ...
    if (isFavorite) {
      ...
      await supabase
        .from("favorites")
        .insert({ user_id: session.user.id, place_id: place?.id });
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        bookingDate: checkIn ? checkIn.split("T")[0] : null,
        bookingTime: checkIn && checkIn.includes("T") ? checkIn.split("T")[1] : null,
      ...
      toast.error(error?.message || "حدث خطأ أثناء الحجز، حاول مرة أخرى");
    } finally {
      setBookingLoading(false);
    }
  };
  ...
  const nextImage = () => {
    if (place?.media_urls && currentImageIndex < place.media_urls.length - 1) {
  ...
  if (!place) {
    return <div className="text-white text-center p-20">عفواً، المكان غير موجود</div>;
  }
    ...
    <main className={`min-h-screen bg-[#0a0a0a] text-white pb-24 ${tajawal.className}`} dir="rtl">
      ...
      <div className="relative h-[60vh] w-full group">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {favLoading ? (
                ...
                <Heart size={18} className={isFavorite ? "fill-red-500 text-red-500 animate-in zoom-in" : ""} />
              )}
            </button>
        ...
        <img
          ...
          alt={place.name}
          onClick={() => {
            if (place.media_urls?.length) openModal(0);
          }}
        ...
        <div
          className={`absolute bottom-3 right-3 md:bottom-4 md:right-4 backdrop-blur text-white text-[9px] md:text-[10px] px-2 py-1 rounded-md md:rounded-lg font-bold shadow-lg z-10 ${
            isBooking && place.price && place.price > 0 ? "bg-[#C89B3C]/90 text-black" : "bg-emerald-500/90"
          }`}
        >
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pointer-events-auto">
              <div>
                <div className="flex gap-2 mb-3">
                  <span className="bg-[#C89B3C] text-black text-xs font-bold px-3 py-1 rounded-full shadow-lg">
                    {place.subtype || (place.type === "accommodation" ? "سكن" : place.type === "experience" ? "تجربة" : "معلم")}
                  </span>
                  {place.city && (
                    <span className="bg-white/20 backdrop-blur-md text-white text-xs px-3 py-1 rounded-full border border-white/10 shadow-lg">
                      {place.city}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  ...
                  <button
                    ...
                    className="text-xs bg-white/10 hover:bg-white/20 text-white px-3 py-1.5 rounded-lg transition border border-white/10 flex items-center gap-1 shadow-md"
                  >
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-white/5 shadow-xl">
              <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
                <AlertCircle className="text-[#C89B3C]" size={24} />
                عن {place.type === "experience" ? "التجربة" : "المكان"}
              </h2>
                ...
                <div className="flex gap-6 mt-6 p-4 bg-black/30 rounded-2xl border border-white/5">
                  {place.duration && (
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#C89B3C]/10 flex items-center justify-center text-[#C89B3C]">
                        <Clock size={20} />
                      </div>
                      <div>
                        <div className="text-xs text-gray-400">المدة</div>
                        <div className="font-bold">{place.duration}</div>
                      </div>
                    </div>
                  )}
                  ...
                  {place.difficulty && (
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="font-bold">{place.difficulty}</div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {place.media_urls && place.media_urls.length > 1 && (
              <div className="bg-[#1a1a1a] p-6 rounded-3xl border border-white/5 shadow-xl">
                <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                  <ImageIcon className="text-[#C89B3C]" size={20} />
                  معرض الصور
                </h2>
                <div className="flex overflow-x-auto gap-4 pb-2 custom-scrollbar">
                  {place.media_urls.map((url, idx) => (
                    <div
                      ...
                      className="w-32 h-32 shrink-0 rounded-xl overflow-hidden cursor-pointer group border border-transparent hover:border-[#C89B3C] transition-all relative"
                    >
                      <img
                        src={url}
            ...
            {isBooking ? (
              <div className="sticky top-24 bg-[#1a1a1a] border border-[#C89B3C]/30 p-6 rounded-3xl shadow-2xl shadow-[#C89B3C]/10">
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="bg-black/30 border border-white/10 rounded-xl flex items-center relative overflow-hidden group hover:border-[#C89B3C]/50 transition">
                        ...
                        <input
                          ...
                          className="w-full bg-transparent p-3 pr-10 outline-none text-white text-sm cursor-pointer"
                        />
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className={`w-full bg-[#C89B3C] text-black font-bold py-4 rounded-xl hover:bg-[#b38a35] transition shadow-lg shadow-[#C89B3C]/20 mt-4 flex justify-center items-center gap-2 ${bookingLoading ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
              ...
              <div className="sticky top-24 bg-[#1a1a1a] border border-white/10 p-6 rounded-3xl shadow-xl">
                <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <Compass className="text-[#C89B3C]" size={24} />
                  استكشف المكان
                </h3>
                <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                  هذا المعلم مفتوح للزيارة العامة ولا يتطلب حجزاً مسبقاً. احصل على الاتجاهات الآن وابدأ رحلتك.
                </p>
                <button
                  onClick={handleOpenLocation}
                  className="w-full bg-white/10 text-white font-bold py-4 rounded-xl hover:bg-white/20 transition flex justify-center items-center gap-2 border border-white/5"
                >
                  <MapPin size={20} />
      ...
      {isImageModalOpen && place?.media_urls && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col items-center justify-center animate-in fade-in duration-300">
          <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center z-10 bg-gradient-to-b from-black/80 to-transparent">
            ...
            <div className="bg-black/50 px-4 py-2 rounded-full font-bold tracking-widest border border-white/10 shadow-lg">
              {currentImageIndex + 1} / {place.media_urls.length}
            </div>
          </div>

          <div className="relative w-full h-full flex items-center justify-center p-4 md:p-12">
            <img
              src={place.media_urls[currentImageIndex]}
              alt="Fullscreen View"
          ...
          {place.media_urls.length > 1 && (
            <>
              <button
                ...
                className={`absolute right-4 md:right-10 top-1/2 -translate-y-1/2 p-4 bg-white/10 hover:bg-[#C89B3C] hover:text-black rounded-full transition backdrop-blur-md border border-white/10 ${currentImageIndex === 0 ? "opacity-30 cursor-not-allowed" : "cursor-pointer"}`}
              >
              ...
              <button
                onClick={nextImage}
                disabled={currentImageIndex === place.media_urls.length - 1}
                className={`absolute left-4 md:left-10 top-1/2 -translate-y-1/2 p-4 bg-white/10 hover:bg-[#C89B3C] hover:text-black rounded-full transition backdrop-blur-md border border-white/10 ${currentImageIndex === place.media_urls.length - 1 ? "opacity-30 cursor-not-allowed" : "cursor-pointer"}`}
              >
                <ChevronLeft size={32} />
              </button>
            </>
