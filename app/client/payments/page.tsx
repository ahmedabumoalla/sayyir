...
import { CreditCard, ArrowDownLeft, Calendar, CheckCircle2, XCircle, Clock, Wallet, Loader2 } from "lucide-react";
...
export default function ClientPaymentsPage() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'succeeded': return { icon: <CheckCircle2 size={16} />, color: "text-emerald-400", bg: "bg-emerald-400/10", label: "ناجحة" };
      case 'failed': return { icon: <XCircle size={16} />, color: "text-red-400", bg: "bg-red-400/10", label: "فشلت" };
      case 'pending': return { icon: <Clock size={16} />, color: "text-amber-400", bg: "bg-amber-400/10", label: "معالجة" };
      case 'refunded': return { icon: <ArrowDownLeft size={16} />, color: "text-blue-400", bg: "bg-blue-400/10", label: "مسترجعة" };
      default: return { icon: <CreditCard size={16} />, color: "text-gray-400", bg: "bg-gray-400/10", label: status };
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1">
           <h2 className="text-2xl font-bold text-white mb-2 flex items-center gap-2">
             <CreditCard className="text-[#C89B3C]" /> سجل المدفوعات
           </h2>
        ...
        <div className="bg-gradient-to-br from-[#C89B3C]/20 to-[#C89B3C]/5 border border-[#C89B3C]/20 rounded-2xl p-5 min-w-[250px] flex items-center gap-4">
          <div className="p-3 bg-[#C89B3C] rounded-full text-[#2B1F17]"><Wallet size={24} /></div>
          <div>
      ...
      <div className="bg-[#252525] border border-white/10 rounded-3xl overflow-hidden shadow-xl">
        {payments.length === 0 ? (
                  ...
                  return (
                    <tr key={payment.id} className="hover:bg-white/5 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-bold text-white">{payment.description || "حجز خدمة"}</div>
                        <div className="text-xs text-white/40 font-mono">ID: {payment.id.slice(0, 8)}...</div>
                      </td>
                      <td className="px-6 py-4 font-bold font-mono text-lg text-[#C89B3C]">
                      ...
                      <td className="px-6 py-4 text-white/60">{payment.payment_method}</td>
                      <td className="px-6 py-4 text-white/50 flex items-center gap-2">
                        <Calendar size={14} /> {new Date(payment.created_at).toLocaleDateString('ar-SA')}
                      </td>
                    </tr>
                  );
                })}
