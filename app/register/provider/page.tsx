...
export default function DynamicProviderRegister() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pendingPolicies = fields.filter(
      (f) => f.field_type === "policy" && answers[f.id] !== true
    );
    ...
    try {
      ...
      fields.forEach((f) => {
        ...
        if (f.label.includes("اسم") || f.field_type === "text") {
          if (!nameVal || nameVal === "مزود جديد") nameVal = val;
        }
      ...
      if (
        ...
        alert("❌ هذا المستخدم مسجل كمزود خدمة مسبقاً.");
        setSubmitting(false);
        return;
      }
      ...
      const { data: requestByPhone } = await supabase
        .from("provider_requests")
        .select("status")
        .eq("phone", normalizedPhone)
        .in("status", ["pending", "approved"]);
      ...
      if (mergedRequests.length > 0) {
        const isApproved = mergedRequests.some((r) => r.status === "approved");
        if (isApproved) {
          alert("❌ هذا المستخدم مسجل كمزود خدمة مسبقاً.");
        } else {
          alert(
            "❌ يوجد طلب انضمام تحت الإجراء لهذا المستخدم. يرجى انتظار رد الإدارة."
          );
        }
        setSubmitting(false);
        return;
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#1a1a1a] text-[#C89B3C]">
  ...
  return (
    <div
      className={`relative min-h-screen w-full bg-[#121212] text-white ${tajawal.className}`}
      dir="rtl"
    >
      ...
      <div className="relative z-10 container mx-auto px-4 py-12 flex flex-col items-center">
        <Link
          href="/"
          className="mb-8 hover:scale-105 transition duration-300 drop-shadow-lg"
        >
        ...
        <div className="w-full max-w-2xl animate-in slide-in-from-bottom-8 duration-700">
          <div className="text-center mb-8">
            ...
            <p className="text-white/60">
              سجل بياناتك وانضم لنخبة مزودي الخدمات في عسير
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                ...
                {field.field_type === "text" && (
                  <div className="relative">
                    ...
                    />
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 opacity-0 group-focus-within:opacity-100 transition duration-500 text-[#C89B3C]">
                      <Check size={18} />
                    </div>
                  </div>
                )}
                ...
                {field.field_type === "email" && (
                  <div className="relative">
                    <input
                      ...
                      dir="ltr"
                    />
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 opacity-0 group-focus-within:opacity-100 transition duration-500 text-[#C89B3C]">
                      <Check size={18} />
                    </div>
                  </div>
                )}
                  ...
                  <div className="relative flex flex-row-reverse" dir="ltr">
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        const justNumber = currentVal.startsWith("+")
                          ? currentVal.replace(/^\+\d+\s*/, "")
                          : currentVal;
                        handleChange(field.id, e.target.value + justNumber);
                      }}
                    >
                      <option value="+966" className="bg-[#1a1a1a]">
                        +966 🇸🇦
                      </option>
                      <option value="+971" className="bg-[#1a1a1a]">
                        +971 🇦🇪
                      </option>
                      <option value="+965" className="bg-[#1a1a1a]">
                        +965 🇰🇼
                      </option>
                      <option value="+973" className="bg-[#1a1a1a]">
                        +973 🇧🇭
                      </option>
                      <option value="+968" className="bg-[#1a1a1a]">
                        +968 🇴🇲
                      </option>
                      <option value="+974" className="bg-[#1a1a1a]">
                        +974 🇶🇦
                      </option>
                ...
                {field.field_type === "select" && (
                  <div className="relative">
                    <button
                      ...
                      onClick={() =>
                        setOpenSelectId(
                          openSelectId === field.id ? null : field.id
                        )
                      }
                      className={`w-full bg-white/5 border rounded-2xl px-5 py-4 text-right flex justify-between items-center transition-all ${
                        openSelectId === field.id
                          ? "border-[#C89B3C] bg-white/10"
                          : "border-white/10"
                      }`}
                    >
                      <span
                        className={answers[field.id] ? "text-white" : "text-white/40"}
                      >
                        {answers[field.id] || "اختر من القائمة..."}
                      </span>
                      <ChevronDown
                        size={20}
                      ...
                      <div className="absolute top-full mt-2 w-full bg-[#1F1F1F] border border-white/10 rounded-xl shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        {field.options?.map((opt: string) => (
                          <div
                            ...
                            className="px-5 py-3 hover:bg-[#C89B3C]/20 hover:text-[#C89B3C] cursor-pointer transition border-b border-white/5 last:border-0 text-sm"
                          >
                            {opt}
                          </div>
                        ))}
                    ...
                    >
                      ...
                      >
                        <div className="group cursor-grab active:cursor-grabbing">
                          <MapPin
                            size={48}
                            className="text-[#C89B3C] drop-shadow-2xl -mt-10 transition-transform hover:scale-110"
                            fill="#2B1F17"
                          />
                        </div>
                      </Marker>
                ...
                {field.field_type === "file" && (
                  <div className="relative group/upload">
                    ...
                    <label
                      ...
                      className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-white/10 bg-white/5 rounded-3xl cursor-pointer hover:border-[#C89B3C] hover:bg-[#C89B3C]/5 transition-all duration-300"
                    >
                      <div className="p-4 rounded-full bg-white/5 mb-3 group-hover/upload:scale-110 transition">
                        <UploadCloud size={30} className="text-[#C89B3C]" />
                      </div>
                      <span className="text-sm text-white/60">
                        اضغط لرفع الصور أو الفيديو
                      </span>
                    </label>
                    ...
                    {previews[field.id]?.length > 0 && (
                      <div className="flex gap-3 mt-4 overflow-x-auto pb-2 custom-scrollbar">
                        {previews[field.id].map((src, idx) => {
                          const fileObj = files[field.id]?.[idx];
                          const isVideo = fileObj?.type?.startsWith("video/");

                          return (
                            <div
                              key={idx}
                              className="w-20 h-20 rounded-xl overflow-hidden relative shrink-0 border border-white/20 shadow-lg group-hover:border-[#C89B3C]/50 transition bg-black/20"
                            >
                              {isVideo ? (
                                <video
                                  src={src}
                ...
                {field.field_type === "policy" && (
                  <div className="bg-gradient-to-r from-[#C89B3C]/10 to-transparent border border-[#C89B3C]/20 rounded-2xl p-5 flex items-start gap-4 transition hover:bg-[#C89B3C]/15">
                    <div className="relative pt-1">
                      ...
                      <Check
                        size={16}
                        className="absolute top-[6px] left-[4px] text-black opacity-0 peer-checked:opacity-100 pointer-events-none transition-opacity font-bold"
                      />
                    </div>
                    <div className="flex-1">
                      ...
                      {field.options && field.options[0] && (
                        <button
                          type="button"
                          onClick={() => openPolicyModal(field.label, field.options[0])}
                          className="mt-2 text-xs text-[#C89B3C] hover:text-white flex items-center gap-1 transition-colors font-medium border-b border-dashed border-[#C89B3C]/50 w-fit pb-0.5"
                        >
                          <FileText size={14} />
                          اقرأ الشروط والأحكام الكاملة
                        </button>
                      )}
                    </div>
                  </div>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full bg-gradient-to-r from-[#C89B3C] to-[#b38a35] text-[#2B1F17] py-5 rounded-2xl font-bold text-lg hover:shadow-[0_0_30px_rgba(200,155,60,0.4)] transition transform hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 mt-8"
            >
              {submitting ? (
                <Loader2 className="animate-spin" />
              ) : (
                <>
                  إرسال الطلب <ArrowRight size={20} className="rotate-180" />
                </>
              )}
            </button>
          </form>
          ...
          <div className="bg-[#1a1a1a] w-full max-w-2xl rounded-3xl border border-white/10 shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-[#C89B3C]/20 to-transparent">
              <h3 className="font-bold text-xl text-white flex gap-2 items-center">
                <FileText size={24} className="text-[#C89B3C]" />
                الشروط والأحكام
              </h3>
              <button
            ...
            <div className="p-8 overflow-y-auto custom-scrollbar text-white/80 leading-loose text-justify whitespace-pre-line text-sm">
              {currentPolicyText}
            </div>
